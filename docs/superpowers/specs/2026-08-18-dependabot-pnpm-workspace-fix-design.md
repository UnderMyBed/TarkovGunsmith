...
the npm ecosystem. **All 7 npm PRs fail CI.** Each one edits only `package.json` and never
`pnpm-lock.yaml`, so `pnpm install --frozen-lockfile` refuses:
...
The 5 `github-actions` PRs are green — that ecosystem touches only workflow YAML and has no
lockfile, so it is unaffected.
...
5. The 5 green `github-actions` PRs are merged.
6. The 7 broken npm PRs are closed, and Dependabot's re-run after the config change produces npm
   PRs that include `pnpm-lock.yaml` and pass CI.
