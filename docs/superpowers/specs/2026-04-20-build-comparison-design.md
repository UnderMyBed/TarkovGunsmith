...
New KV key prefix: `p:$id` (alongside the existing `b:$id` build namespace — same KV binding). 8-character IDs generated by the existing `newBuildId()` / `BUILD_ID_REGEX` machinery. TTL matches single builds: `env.BUILD_TTL_SECONDS` (30 days today).

Worker routes (the Pages Function at `apps/web/functions/api/builds/[[path]].ts` gets a sibling `.../api/pairs/[[path]].ts` that strips `/api` and forwards the same way):

- `POST /pairs` — body is `BuildPair` JSON (≤32 KB), returns `201 { id, url }`.
- `GET /pairs/:id` — returns the stored `BuildPair` JSON or `404`. `400` on bad id.
- `POST /pairs/:id/fork` — server-side copy-on-write helper. Returns `201 { id, url }` with a copy of the stored pair. Used by "Save as new" from an already-loaded pair to avoid a full-blob round trip.

All three routes reuse the existing `readBody` + JSON-parse + id-regex gates from single-build routes. No new env vars, no new secrets, same KV namespace (`BUILDS`), same TTL, same body-size limit.
...
Lives in `packages/tarkov-data/src/build-schema.ts` next to `BuildV4`, re-exported from the package `index.ts`:
...
New hooks and API client fns mirroring existing single-build patterns in `packages/tarkov-data/src/buildsApi.ts` + `packages/tarkov-data/src/hooks/useLoadBuild.ts` / `useSaveBuild.ts`:

- `savePair(fetchImpl, pair)` / `loadPair(fetchImpl, id)` / `forkPair(fetchImpl, id)` — client fns in `packages/tarkov-data/src/pairsApi.ts`. Base endpoint `/api/pairs`. Share the `BUILD_ID_REGEX` constant (re-export).
- `useSavePair()` — `UseMutationResult<SavePairResponse, Error, BuildPair>`.
- `useLoadPair(id)` — `UseQueryResult<BuildPair, Error>` keyed on `["pair", id]`. Disabled on empty id.
- `useForkPair()` — `UseMutationResult<SavePairResponse, Error, string>` (body is pair id).
- `LoadPairError extends Error` class with `code: LoadPairErrorCode` exactly like `LoadBuildError`.

`LoadPairError` taxonomy mirrors the existing `LoadBuildError` class in `packages/tarkov-data/src/buildsApi.ts` exactly — same 4 codes:

- `invalid-id` — id doesn't match the 8-char safe-alphabet regex.
- `not-found` — 404 from Worker (expired past 30-day TTL or never existed).
- `unreachable` — network failure or non-200/404 status.
- `invalid-schema` — Zod parse failure on retrieved payload.

Upstream drift — embedded builds referencing items that no longer exist in the current tarkov-dev data — is **not** a `LoadPairError`. It's handled post-load the same way single builds handle it: the loader succeeds, the workspace renders, and `OrphanedBanner` appears over any side whose mod tree has orphaned items.
...
- `LoadPairError` typed taxonomy (4 codes from 4.5). Loader route renders appropriate error UI per code:
  - `invalid-id` → error shell explaining the id format, with "Start a new comparison" CTA to `/builder/compare`.
  - `not-found` → 404 shell with "This comparison has expired (30-day lifetime) or never existed" + "Start a new comparison" CTA.
  - `unreachable` → error shell with retry button (reuses `useLoadPair.refetch()`).
  - `invalid-schema` → error shell with raw error code + "Start a new comparison" CTA.
- Upstream drift on successful load → workspace loads; `OrphanedBanner` renders over affected side(s). See §6.
