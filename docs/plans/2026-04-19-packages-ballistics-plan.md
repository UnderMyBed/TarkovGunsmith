...
- Cross-check fixtures vs. the original `WishGranter` C# outputs are not yet present. **Correction (2026-08-19):** the claim that the C# repo is "archived as defunct" is false — `Xerxes-17/TarkovGunsmith` is live and was pushed to as recently as 2025-12-17. The armor math is in `BackEnd/WishGranter/Statics/Ballistics.cs` and was read directly during the 2026-08 data audit, which found our durability model wrong by 35–58×. See `docs/operations/data-api-audit.md` §G.
